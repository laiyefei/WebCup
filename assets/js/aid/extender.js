...
;(function (window) {
    ...
    var extender = {
        ...
        GlobalCache: {
            ...
            //get
            get: function (key) {
                return extender["GlobalCache"]["storage"][key];
            },
            //save
            save: function (key, value) {
                //todo..check

                extender["GlobalCache"]["storage"][key] = value;
                return true;
            },
            //exist
            exist: function (key) {
                return extender["GlobalCache"]["storage"][key] != undefined;
            },
            remove: function (key) {
                if (extender["GlobalCache"]["storage"][key] == undefined) {
                    console.warn("this key [" + key + "] is not exist.");
                    return false;
                }
                extender["GlobalCache"]["storage"][key] = undefined;
                return true;
            }
        }
        ,

        cloneArray:function(tag){
            
            if(!Array.isArray(tag)){
                console.error("sorry, the extender.cloneArray you invoke param must be Array.");
                return null;
            }
            var clone = [];
        ...
        jsmsgSuccess: function (msgtitle, callback) {
            extender.jsmsg(msgtitle, 'Success', callback);
        },
        //错误消息提醒
        jsmsgError: function (msgtitle, callback) {
            extender.jsmsg(msgtitle, 'Error', callback);
        },
        //表单验证
        vaildform: function (jqform, vaildHidden) {
            jqform = jqform || $("#baseAttrForm");
            if (extender.checkFlag == 0) {
            ...
            $(nodes).each(function (index, item) {
                ...
                $(item).children().each(function (index1, item1) {
                    if ($(item1).children().length > 0) {
                        treeData = extender.node2elementtree(id, id, item1, treeData);
                    } else {
                        id = id + 1;
        ...
        xml2elementtree: function (strXml) {
            var treeData = [];
            var xmlObj = extender.xml2xmlObject(strXml);
            var xmlDoc = xmlObj.documentElement;
            var id = 0;
            var pid = 0;
            treeData = extender.node2elementtree(id, pid, xmlDoc, treeData);
            return treeData;
        },
